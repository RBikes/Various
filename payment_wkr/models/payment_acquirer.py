...
class TransferPaymentAcquirer(osv.Model):
    ...
    def _format_transfer_data(self, cr, uid, context=None):
        ...
        post_msg = '''<div>
<h4>You will receive an order confirmation at the email address specified. Please submit this order confirmation to your WKR provider (FiscFree/BFN).
As soon as we receive confirmation from your WKR provider, the production of your bike or shipment of your accessory will start!</h4>
<p>Please use the order number as reference in any future communication.</p>
</div>'''
        return post_msg
