...
class delivery_carrier(orm.Model):
    ...
    _columns = {
        ...
        'via_reseller': fields.boolean('Deliver via Reseller', copy=False),
    }
    _defaults = {
...
class SaleOrder(orm.Model):
    ...
    def _get_delivery_methods(self, cr, uid, order, context=None):
        ...
        for delivery_id in carrier_obj.browse(cr, SUPERUSER_ID, delivery_ids, context=dict(context, order_id=order.id)):
            ...
            if shipping_id == -0.5:
                if not delivery_id.via_reseller:
                    delivery_ids.remove(delivery_id.id)
        return delivery_ids
    ...
    def _get_errors(self, cr, uid, order, context=None):
        ...
        if not self._get_delivery_methods(cr, uid, order, context=context):
            errors.append(('From abroad are you?', 'Please contact us at info@roetz-bikes.nl or +31208200804 so we can provide you with a tailor made shipping proposition.'))            
        return errors
