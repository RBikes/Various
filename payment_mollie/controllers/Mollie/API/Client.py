...
class Client:
    ...
    def performHttpCall(self, http_method, path, data=None, params=None):
        ...
        try:
            response = requests.request(http_method, url, verify=True, headers={'Accept': 'application/json', 'Authorization': 'Bearer ' + self.api_key, 'User-Agent': user_agent, 'X-Mollie-Client-Info': uname}, params=params, data=data)
