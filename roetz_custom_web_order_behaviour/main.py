...
class PaymentTransaction(orm.Model):
    ...
    def form_feedback(self, cr, uid, data, acquirer_name, context=None):
        """ Override to confirm the sale order and send a confirmation, if the transaction
        is done. """
        tx = None
        ...
        if tx and tx.state == 'done' and tx.sale_order_id and tx.sale_order_id.state in ['draft', 'sent']:
            ...
            self.pool['sale.order'].action_quotation_send(cr, SUPERUSER_ID, [tx.sale_order_id.id], context=context)
